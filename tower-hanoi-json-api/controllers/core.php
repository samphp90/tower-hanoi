...
class JSON_API_Core_Controller {
  ...
  public function tawor_hanoi(){
    global $json_api;
    if(isset($_GET['disk']) && $_GET['disk'] != '' && $_GET['disk'] >0){
      $disk = $_GET['disk'];
      $time = microtime(true);
      $hanoi = new Hanoi($disk, Hanoi::PEG_A);
      $hanoi->setDebug(true);
      $hanoiSolving = new HanoiRecursiveSolving($hanoi);
      $hanoiSolving->solve($hanoi->getNumberOfDisks(), $hanoi->getStartPosition(), Hanoi::PEG_C);

      $result['count'] = $hanoi->getCountMove();
      $i = 0;
      foreach ($hanoi->getDebugInfo() as $infoMessage) {
          $result['steps'][$i++] = $infoMessage;
      } 

      return $result;
    }else{
      $json_api->error("Include 'Disk' var in your request.");
    }
